function gen(files) {
    ...
    $.post("http://192.168.18.188:19081/data/object/attribute/list",
        {resId:tableString[0],page:1},
        function (resp) {
            if (resp.success) {
...
function tables() {
    ...
    $.post("http://192.168.18.188:19081/resource/get",
        {
            dataObjectId:'',
            keyword:'',
            page:1,
        ...
        function (resp) {
            if (resp.success) {
                $.each(resp.data.list, function (idx, table) {
                    console.log(resp);
                    selectObj.append('<option value="' + table.id+","+table.defined + '">' + table.name + '</option>');
                });
